...
class FileIO(object):
    ...
    def loadDataSet(conf, file, bTest=False,binarized = False, threshold = 3.0):
        ...
        for lineNo, line in enumerate(ratings):
            diseases = split(delim,line.strip())
            if not bTest and len(order) < 2:
                print('The rating file is not in a correct format. Error: Line num %d' % lineNo)
                exit(-1)
            try:
                drugId = diseases[int(order[0])]
                diseaseId = diseases[int(order[1])]
                if len(order)<3:
                    rating = 1 #default value
                else:
                    rating  = diseases[int(order[2])]
                if binarized:
                    if float(diseases[int(order[2])])<threshold:
                        continue
                    else:
                        rating = 1
            except ValueError:
                ...
                exit(-1)
            if bTest:
                testData.append([drugId, diseaseId, float(rating)])
            else:
                trainingData.append([drugId, diseaseId, float(rating)])
        if bTest:
            return testData
        else:
            return trainingData
